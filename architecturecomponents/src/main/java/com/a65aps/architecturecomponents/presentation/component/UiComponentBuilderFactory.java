...
public class UiComponentBuilderFactory<Key, ComponentBuilder>
    ...
    @Inject
    public UiComponentBuilderFactory(
            @NonNull Map<Key, Provider<ComponentBuilder>> subComponentBuilders) {
    ...
    public ComponentBuilder get(@NonNull Key key) {
        ...
        if (provider == null) {
            throw new AssertionError("Can't find provider for component key = "
                    + key.toString());
        }
